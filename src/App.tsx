...
function App() {
  ...
  const handleFileUpload = async (file: File, type: "format" | "paper") => {
    try {
      // Use Rust backend to parse the document
      // First, we need to get the file path - in Tauri, we'd use dialog to get the actual path
      // For now, let's use the invoke command to parse
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-primary-500 transition-colors">
              <input
                ...
                onChange={(e) => {
                  const file = e.target.files?.[0];
                  if (file) handleFileUpload(file, "paper");
                }}
                className="hidden"
                id="paper-file"
              />
