...
function App() {
  ...
  const [apiModel, setApiModel] = useState("MiniMax-Text-01");
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="grid md:grid-cols-2 gap-6 mb-6">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center hover:border-primary-500 transition-colors">
              <input
                ...
                onChange={async (e) => {
                  ...
                  if (file) {
                    try {
                      // Use Tauri dialog to get file path and parse with Rust backend
                      ...
                      if (selected && typeof selected === "string") {
                        const fileName = selected.split("/").pop() || "未知文件";
                        // Parse document using Rust backend to get format info
                        const parsed = await invoke<ParsedDocument>("parse_document", { filePath: selected });
                        setFormatFile({ name: fileName } as File);
                        setFormatDoc(parsed);
                      } else {
                        // Fallback: if dialog cancelled, use file input (text only)
                      ...
                      console.error("Parse failed, using text fallback:", err);
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="border-2 border-dashed border-gray-300 rounded-lg p-8 text-center">
              {paperFile ? (
                <div>
                  ...
                  <p className="text-sm mt-1">点击下方按钮重新选择</p>
                </div>
              ) : (
                <div>
                  <p className="text-4xl mb-2">📝</p>
            ...
            <button
              onClick={async () => {
                try {
                  ...
                  if (selected && typeof selected === "string") {
                    ...
                    setPaperFile({ name: fileName } as File);
                    setPaperDoc(parsed);
                  }
                } catch (err) {
              ...
              className="mt-4 w-full py-2 px-4 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors"
            >
              {paperFile ? "重新选择文件" : "选择文件"}
            </button>
          </div>
        </div>
        ...
        {checkResult && (
          <div className="bg-white rounded-lg shadow p-6">
            ...
            {/* Format Comparison - Before Summary */}
            <div className="grid grid-cols-2 gap-6 mb-6">
              {/* Format Requirements */}
              <div className="bg-gray-50 rounded-lg p-4">
                <h3 className="font-medium text-gray-700 mb-2">格式要求 (来自规范文档)</h3>
                <div className="text-sm text-gray-600 max-h-48 overflow-y-auto whitespace-pre-wrap">
                  {formatDoc?.content?.slice(0, 2000)}
                  {(formatDoc?.content?.length || 0) > 2000 && "..."}
                </div>
              </div>

              {/* Paper Format Info */}
              <div className="bg-gray-50 rounded-lg p-4">
                <h3 className="font-medium text-gray-700 mb-2">论文格式信息</h3>
                <div className="text-sm text-gray-600 space-y-1">
                  <p>📄 总页数: {paperDoc?.metadata.page_count || "-"}</p>
                  <p>📝 总字数: {paperDoc?.metadata.word_count || "-"}</p>
                  <p>📊 段落数: {paperDoc?.paragraphs.length || "-"}</p>
                  {(paperDoc?.format_info?.fonts?.length || 0) > 0 && (
                    <p>🔤 字体: {paperDoc?.format_info.fonts.join(", ")}</p>
                  )}
                  {(paperDoc?.format_info?.styles?.length || 0) > 0 && (
                    <>
                      <p className="font-medium mt-2">样式列表:</p>
                      {paperDoc?.format_info?.styles?.slice(0, 10).map((style, i) => (
                        <p key={i} className="ml-2">
                          - {style.name}: {style.font_name || ""} {style.font_size ? `${style.font_size}pt` : ""}
                        </p>
                      ))}
                      {(paperDoc?.format_info?.styles?.length || 0) > 10 && (
                        <p className="ml-2 text-gray-400">... 共 {paperDoc?.format_info?.styles?.length} 种样式</p>
                      )}
                    </>
                  )}
                </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    // Auto-change model based on provider
                    if (newProvider === "minimax") {
                      setApiModel("MiniMax-Text-01");
                    } else {
              ...
              <div>
                ...
                >
                  {apiProvider === "minimax" ? (
                    <>
                      <option value="MiniMax-Text-01">MiniMax-Text-01 (推荐)</option>
